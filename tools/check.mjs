...
async function checkPackage() {
  ...
  assert(pkg.exports?.['.']?.import === './dist/index.mjs', 'root export must use dist.');
  ...
  assert(pkg.exports?.['./components/DdysView']?.types === './src/runtime/components/DdysView.astro.d.ts', 'component typed export mismatch.');
  assert(pkg.exports?.['./components/*']?.types === './src/runtime/components/*.astro.d.ts' && pkg.exports?.['./components/*']?.import === './dist/runtime/components/*.astro', 'component export pattern mismatch.');
  assert(pkg.exports?.['./styles.css'] === './dist/runtime/styles/ddys.css', 'styles export mismatch.');
  assert(pkg.files?.includes('dist') && pkg.files?.includes('src') && pkg.files?.includes('components') && pkg.files?.includes('examples') && pkg.files?.includes('public'), 'package files must include dist, src, component declarations, examples, and public.');
...
async function checkIntegration() {
  ...
  for (const fragment of ['AstroIntegration', 'astro:config:setup', 'injectRoute', 'injectScript', 'addMiddleware', 'createCodegenDir', 'virtual:ddys-astro/config', '/sitemap.xml', '/manifest.webmanifest', 'runtime/pages/movie/[slug]/sources.astro']) {
...
async function checkEndpoints() {
  for (const file of ['proxy.ts', 'request.ts', 'diagnostics.ts', 'revalidate.ts', 'sitemap.ts', 'robots.ts', 'manifest.ts', 'assets.ts']) {
...
async function checkExamples() {
  ...
  assert((await read('examples/astro-app/src/content/config.ts')).includes('createDdysContentLoader'), 'content loader example missing.');
}
...
async function checkDocs() {
  ...
  for (const fragment of ['ddys-astro', 'astro:config:setup', 'Content Loader', 'DdysRequestForm', 'createDdysContentLoader', '/api/ddys/proxy', 'DDYS_API_KEY']) {
